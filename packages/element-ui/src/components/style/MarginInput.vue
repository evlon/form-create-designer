...
export default defineComponent({
    ...
    setup(props, { emit }) {
        const parseValue = (value) => {
            ...
            const match = value.match(/margin:\s*([^;]+)/);
            if (match) {
        ...
        const buildValue = (state) => {
            if (state.linked) {
                ...
                return `margin: ${val}px`;
            } else if (state.top === state.bottom && state.right === state.left) {
                return `margin: ${state.top}px ${state.right}px`;
            } else {
                return `margin: ${state.top}px ${state.right}px ${state.bottom}px ${state.left}px`;
            }
        };
...
._fd-margin-input .el-input-number {
    width: 100%;
}

._fd-margin-input .label {
    text-align: center;
    font-size: 12px;
    color: #909399;
    margin-top: 2px;
